	...
	if (!_particles.empty())
	{
		...
		for (i = _particles.begin(); i != _particles.end(); i++)
		{
			if (i->_isAlive)
			{
				...
				//
				v->_cameraPositionition = i->_cameraPositionition;
...
void Snow::resetParticle(Attribute* attribute)
{
	...
	d3d::GetRandomVector(
		&attribute->_cameraPositionition,
		_boundingBox.getBoudingBoxMin(),
		_boundingBox.getBoudingBoxMax());

	// no randomness for height (y-coordinate).  Snow flake
	// always starts at the top of bounding box.
	attribute->_cameraPositionition.y = _boundingBox.getBoudingBoxMax()->y;
...
void Snow::update(float timeDelta)
{
	...
	for (i = _particles.begin(); i != _particles.end(); i++)
	{
		i->_cameraPositionition += i->_velocity * timeDelta;

		// is the point outside bounds?
		if (_boundingBox.isPointInside(i->_cameraPositionition) == false)
		{
...
//{
//	attribute->_isAlive = true;
//	attribute->_cameraPositionition = _origin;
//
...
//	attribute->_cameraPositionition = cameraPos;
//	attribute->_cameraPositionition.y -= 1.0f; // slightly below camera
